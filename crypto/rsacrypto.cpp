...
void RsaCrypto::generatePkey(KeyLength bits, QByteArray pub, QByteArray pri)
{
    // 生成秘钥上下文
...
QByteArray RsaCrypto::enCrypto(QByteArray data)
{
    // 生成秘钥上下文
    ...
    */
    ...
    // 对加密后的数据进行Base64编码
    QByteArray retData = m_base64.enCode(reinterpret_cast<char*>(out), outlen);
...
QByteArray RsaCrypto::deCrypto(QByteArray data)
{
    // 先进行Base64解码
...
QByteArray RsaCrypto::sign(QByteArray data, QCryptographicHash::Algorithm hashAlgo)
{
    // 对hash计算后的数据进行签名

    // hash计算
    QCryptographicHash hs = QCryptographicHash(hashAlgo);
    hs.addData(data);
    QByteArray hashedData = hs.result();
    ...
    ret = EVP_PKEY_CTX_set_signature_md(ctx, m_algorithmMap[hashAlgo]());
    assert(ret);
    ...
    // 对签名后的数据进行Base64编码
    QByteArray signData =m_base64.enCode(reinterpret_cast<char*>(sig), siglen);
    assert(ret);
...
bool RsaCrypto::verify(QByteArray sign, QByteArray data, QCryptographicHash::Algorithm hashAlgo)
{
    // 先对签名后的数据进行Base64解码
